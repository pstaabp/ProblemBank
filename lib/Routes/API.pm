...
post '/authors' => sub { # add a new author
  ...
  my $newAuthor = Model::Author->new(body_parameters->as_hashref);
  my $client = MongoDB->connect('mongodb://localhost');
  my $authors = $client->ns('problemdb.authors');
  
  #debug $authors; 
  my $result = $authors->insert_one($newAuthor);
...
post '/problems' => sub { # add a problem. 
  ...
  my $client = MongoDB->connect('mongodb://localhost');
  my $problem = Model::ProblemList::insert_new_problem($client,body_parameters->as_hashref); 
